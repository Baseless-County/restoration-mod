...
Hooks:PostHook( WeaponTweakData, "init", "SC_weapons", function(self)
		...
		--[[     MGs     ]]--
			...
			--[[     HEAVY SMGs     ]]
				...
				--SECONDARIES
					...
					--Uzi
						--Let's assume it's in .41AE (hell no lol)
						self.uzi.has_description = false
			...
			--[[     LIGHT MGs      ]]
				...
				--IM AT SOUP (KAC ChainSAW)
					...
					self.kacchainsaw.reload_speed_multiplier = 0.99
				...
				--RPK
					...
					self.rpk.reload_speed_multiplier = 1.1
			...
			--[[     HEAVY MGs     ]]
				...
				--PRIMARIES			
					...
					--Brenner 21 (HK21)
						...
						self.hk21.sms = sms_preset.lmg_90
						self.hk21.weapon_movement_penalty = sms_preset.lmg_90
					...
					--M60
						...
						self.m60.sms = sms_preset.lmg_48
						self.m60.weapon_movement_penalty = sms_preset.lmg_48
					...
					--KSP 58
						...
						self.par.reload_speed_multiplier = 1.0
			...
			--[[     MINIGUNS      ]]
				...
				--Minigun
					...
					self.m134.CLIP_AMMO_MAX = 900
					self.m134.NR_CLIPS_MAX = 1
					self.m134.AMMO_MAX = 900
		...
		--[[     SHOTGUNS     ]]--
			...
			--[[     LIGHT SHOTGUNS     ]]
				...
				--PRIMARIES
					...
					--M1014 (In secondary, because yes)
	...
	--[[     CUSTOM BEARDLIB WEAPONS     ]]--
		...
		--[[     CARL'S MODS     ]]--
			...
			if self.mk17 then
						self.mk17.desc_id = "bm_scar_sc_desc"
						self.mk17.has_description = true		
						self.mk17.AMMO_MAX = 120
						self.mk17.use_data.selection_index = 1
						self.mk17.CLIP_AMMO_MAX = 20
						self.mk17.fire_mode_data.fire_rate = 0.1090909
						self.mk17.CAN_TOGGLE_FIREMODE = true
						self.mk17.FIRE_MODE = "auto"
						self.mk17.auto.fire_rate = 0.1090909
						self.mk17.kick = self.stat_info.kick_tables.moderate_right_kick		
						self.mk17.muzzleflash = "effects/payday2/particles/weapons/big_762_auto_fps"
						self.mk17.supported = true
						self.mk17.ads_speed = 0.300
						self.mk17.damage_falloff = {
							start_dist = 2000,
							end_dist = 5000,
							min_mult = 0.53333
						}
						...
						self.mk17.stats_modifiers = nil
						self.mk17.panic_suppression_chance = 0.05
			end

			if self.pecheneg then -- Carl's PKP Pecheneg
				self.pecheneg.categories = {
					"lmg",
					"smg",
					"mmg"
				}
				self.pecheneg.recategorize = { "heavy_mg" }
				self.pecheneg.damage_type = "machine_gun"
				...
				self.pecheneg.AMMO_MAX = 300
				self.pecheneg.kick = self.stat_info.kick_tables.horizontal_recoil_mg
				self.pecheneg.always_use_standing = true
				self.pecheneg.muzzleflash = "effects/payday2/particles/weapons/big_51b_auto_fps"
				self.pecheneg.supported = true
				self.pecheneg.ads_speed = 0.440
				self.pecheneg.damage_falloff = {
					start_dist = 2200,
					end_dist = 7000,
					min_mult = 0.5
				}
				...
				self.pecheneg.stats_modifiers = nil
				self.pecheneg.sounds.spin_start = "wp_m60_reload_lever_release"
				self.pecheneg.spin_up_shoot = true
				self.pecheneg.spin_up_t = 0.1
				self.pecheneg.spin_down_t = 0.00000001
				self.pecheneg.reload_speed_multiplier = 0.81
			...
			if self.mk18s then
						...
						self.mk18s.categories = {
							"assault_rifle"
						}
						self.mk18s.panic_suppression_chance = 0.05
						self.mk18s.supported = true
						self.mk18s.ads_speed = 0.280
						self.mk18s.damage_falloff = {
							start_dist = 2000,
							end_dist = 6500,
							min_mult = 0.4166
						}
						...
						self.mk18s.stats_modifiers = nil
						self.mk18s.timers.reload_not_empty = 1.9 
						self.mk18s.timers.reload_empty = 3.4	
						self.mk18s.timers.reload_exit_empty = 0.35
						self.mk18s.timers.reload_exit_not_empty = 0.65
			end

			if self.drongo then
						self.drongo.use_data.selection_index = 1
						self.drongo.has_description = false		
						...
						self.drongo.fire_mode_data.fire_rate =  0.08
						self.drongo.panic_suppression_chance = 0.05
						self.drongo.kick = self.stat_info.kick_tables.moderate_kick	
						self.drongo.supported = true
						self.drongo.ads_speed = 0.200
						self.drongo.damage_falloff = {
							start_dist = 1900,
							end_dist = 6000,
							min_mult = 0.5
						}
						...
						self.drongo.stats_modifiers = nil
						self.drongo.reload_speed_multiplier = 1.0
			end

			if self.pdr then --Carl's PDR
						self.pdr.has_description = false		
						self.pdr.categories = {
							"assault_rifle"
						}
						self.pdr.CLIP_AMMO_MAX = 30
						self.pdr.AMMO_MAX = 180
						self.pdr.tactical_reload = 1
						self.pdr.fire_mode_data.fire_rate = 0.08
						self.pdr.auto.fire_rate = 0.08
						self.pdr.panic_suppression_chance = 0.05
						self.pdr.kick = self.stat_info.kick_tables.even_recoil
						self.pdr.supported = true
						self.pdr.ads_speed = 0.220
						self.pdr.damage_falloff = {
							start_dist = 1800,
							end_dist = 5000,
							min_mult = 0.3333
						}
		...
		--[[     PAWCIO'S MODS     ]]--
			...
			if self.ppk20 then
						...
						self.ppk20.tactical_reload = 1
						self.ppk20.CAN_TOGGLE_FIREMODE = true
						self.ppk20.fire_mode_data.fire_rate = 0.08
						self.ppk20.kick = self.stat_info.kick_tables.right_recoil
						self.ppk20.supported = true
						self.ppk20.ads_speed = 0.200
						self.ppk20.damage_falloff = {
							start_dist = 1100,
							end_dist = 4000,
							min_mult = 0.2
						}
						...
						self.ppk20.stats_modifiers = nil
						self.ppk20.panic_suppression_chance = 0.05
						self.ppk20.timers.reload_exit_empty = 1.05
						self.ppk20.timers.reload_exit_not_empty = 0.95
			end
		...
		if self.cbjms then
						...
						self.cbjms.desc_id = "bm_sr2_sc_desc"
						self.cbjms.fire_mode_data.fire_rate = 0.075
						...
						self.cbjms.reload_speed_multiplier = 1.02
		end

			if self.mpx then
						self.mpx.CLIP_AMMO_MAX = 30
						self.mpx.fire_mode_data.fire_rate = 0.07058823529
						self.mpx.panic_suppression_chance = 0.05
						self.mpx.kick = self.stat_info.kick_tables.even_recoil
						self.mpx.AMMO_MAX = 150
						self.mpx.tactical_reload = 1
						self.mpx.supported = true
						self.mpx.ads_speed = 0.180
						self.mpx.damage_falloff = {
							start_dist = 900,
							end_dist = 3800,
							min_mult = 0.25
						}
						...
						self.mpx.stats_modifiers = nil
						self.mpx.panic_suppression_chance = 0.05
						self.mpx.timers.reload_exit_empty = 0.6
						self.mpx.timers.reload_exit_not_empty = 0.75
			end
			...
			if self.aek971 then
						...
						self.aek971.ads_speed = 0.250
						self.aek971.damage_falloff = {
							start_dist = 2000,
							end_dist = 5300,
							min_mult = 0.5
						}
			...
			if self.rpd then
					...
					self.rpd.kick = self.stat_info.kick_tables.horizontal_recoil_mg
					self.rpd.always_use_standing = true
					...
					self.rpd.panic_suppression_chance = 0.055
					self.rpd.bipod_req_scope = true
					self.rpd.reload_speed_multiplier = 1.0
			end

			if self.xm8 then --Pawcio's H&K XM8
				self.xm8.recategorize = { "light_ar" }
				self.xm8.damage_type = "assault_rifle"
				self.xm8.tactical_reload = 1
				self.xm8.nato = true
				self.xm8.desc_id = "bm_g36_sc_desc"
				self.xm8.has_description = false				
				self.xm8.fire_mode_data.fire_rate = 0.08
				self.xm8.AMMO_MAX = 210
			...
			if self.mg3 then
					self.mg3.has_description = false		
					self.mg3.categories = {
						"lmg",
						"smg"
					}
					...
					self.mg3.kick = self.stat_info.kick_tables.horizontal_recoil_mg
					self.mg3.always_use_standing = true
					self.mg3.supported = true
					self.mg3.ads_speed = 0.440
					self.mg3.damage_falloff = {
						start_dist = 2200,
						end_dist = 5000,
						min_mult = 0.5
					}
					...
					self.mg3.panic_suppression_chance = 0.055
					self.mg3.bipod_req_scope = true
					self.mg3.reload_speed_multiplier = 1.25
			end
			...
			if self.zweihander then
					...
					self.zweihander.desc_id = "bm_m249_sc_desc"
					self.zweihander.has_description = true
					self.zweihander.CLIP_AMMO_MAX = 120
					self.zweihander.AMMO_MAX = 400
					self.zweihander.fire_mode_data.fire_rate = 0.0545454545454545
					self.zweihander.sms = sms_preset.lmg_40
					self.zweihander.weapon_movement_penalty = sms_preset.lmg_40
					self.zweihander.kick = self.stat_info.kick_tables.horizontal_left_recoil_mg
					self.zweihander.always_use_standing = true
					self.zweihander.muzzleflash = "effects/payday2/particles/weapons/tkb_muzzle"
					self.zweihander.supported = true
					self.zweihander.ads_speed = 0.420
					self.zweihander.damage_falloff = {
						start_dist = 2200,
						end_dist = 7700,
						min_mult = 0.4166
					}
					...
					self.zweihander.shell_ejection = "_dmc/effects/shell_lmg_down"
					self.zweihander.stats_modifiers = nil
					self.zweihander.sounds.spin_start = "wp_m249_lever_release"
					self.zweihander.spin_up_shoot = true
					self.zweihander.spin_up_t = 0.08
					self.zweihander.spin_down_t = 0.00000001
					self.zweihander.timers.equip = 1.2
					self.zweihander.timers.reload_exit_empty = 1.2
					self.zweihander.timers.reload_exit_not_empty = 1.2
			...
			if self.mg4 then  --Pawcio's H&K HK123
					...
					self.mg4.desc_id = "bm_m249_sc_desc"
					self.mg4.has_description = true
					self.mg4.CLIP_AMMO_MAX = 150
					self.mg4.AMMO_MAX = 400
					self.mg4.sms = sms_preset.lmg_48
					self.mg4.weapon_movement_penalty = sms_preset.lmg_48
					self.mg4.fire_mode_data.fire_rate = 0.0705882352941176
					self.mg4.kick = self.stat_info.kick_tables.horizontal_left_recoil_mg
					self.mg4.always_use_standing = true
					self.mg4.muzzleflash = "effects/payday2/particles/weapons/tkb_muzzle"
					self.mg4.supported = true
					self.mg4.ads_speed = 0.420
					self.mg4.damage_falloff = {
						start_dist = 2200,
						end_dist = 7700,
						min_mult = 0.4166
					}
					...
					self.mg4.shell_ejection = "_dmc/effects/shell_lmg_down"
					self.mg4.stats_modifiers = nil
					self.mg4.sounds.spin_start = "wp_m249_lever_release"
					self.mg4.spin_up_shoot = true
					self.mg4.spin_up_t = 0.08
					self.mg4.spin_down_t = 0.00000001
					self.mg4.timers.equip = 1.2
			...
			if self.lsat then
					self.lsat.categories = {
						"lmg",
						"smg"
					}
					self.lsat.desc_id = "bm_m249_sc_desc"
					self.lsat.has_description = true
					self.lsat.CLIP_AMMO_MAX = 100
					self.lsat.AMMO_MAX = 400
					self.lsat.sms = sms_preset.lmg_48
					self.lsat.weapon_movement_penalty = sms_preset.lmg_48
					self.lsat.fire_mode_data.fire_rate = 0.08571428571
					self.lsat.kick = self.stat_info.kick_tables.horizontal_left_recoil_mg
					self.lsat.always_use_standing = true
					self.lsat.muzzleflash = "effects/payday2/particles/weapons/tkb_muzzle"
					self.lsat.supported = true
					self.lsat.ads_speed = 0.400
					self.lsat.damage_falloff = {
						start_dist = 2400,
						end_dist = 7700,
						min_mult = 0.4166
					}
					...
					self.lsat.shell_ejection = "_dmc/effects/shell_lmg_down"
					self.lsat.stats_modifiers = nil
					self.lsat.sounds.spin_start = "wp_m249_lever_release"
					self.lsat.spin_up_shoot = true
					self.lsat.spin_up_t = 0.08
					self.lsat.spin_down_t = 0.00000001
					self.lsat.timers.equip = 1.0
					self.lsat.timers.reload_exit_empty = 1.2
					self.lsat.timers.reload_exit_not_empty = 1.2
					self.lsat.panic_suppression_chance = 0.05
					self.lsat.reload_speed_multiplier = 1.0
			end
		...
		--[[     FrenchyAU's MOD     ]]--
				...
				if self.iso then
						...
						self.iso.auto.fire_rate = 0.0545454
						self.iso.fire_mode_data.fire_rate = 0.0545454
						self.iso.AMMO_MAX = 90
						self.iso.kick = self.stat_info.kick_tables.even_recoil
						self.iso.supported = true
						self.iso.ads_speed = 0.160
						...
						self.iso.timers.reload_exit_empty = 0.6
						self.iso.timers.reload_exit_not_empty = 0.85	
			end
		...
		--[[     MIRA'S MODS     ]]--
			...
			if self.apairofmelons then
						self.apairofmelons.categories = {
							"snp"
						}
						self.apairofmelons.upgrade_blocks = nil
						self.apairofmelons.has_description = true
						self.apairofmelons.desc_id = "bm_ap_weapon_semi_sc_desc"
						self.apairofmelons.CLIP_AMMO_MAX = 20
						self.apairofmelons.use_data.selection_index = 1
						self.apairofmelons.AMMO_MAX = 40
						self.apairofmelons.FIRE_MODE = "single"
						self.apairofmelons.fire_mode_data = {}
						self.apairofmelons.fire_mode_data.fire_rate = 0.1764705
						self.apairofmelons.kick = self.stat_info.kick_tables.vertical_kick
						self.apairofmelons.can_shoot_through_enemy = true
						...
						self.apairofmelons.stats_modifiers = nil
						self.apairofmelons.hs_mult = 2
						self.apairofmelons.keep_ammo = 1
						self.apairofmelons.reload_speed_multiplier = 1.0
						self.apairofmelons.armor_piercing_chance = 1
						self.apairofmelons.panic_suppression_chance = 0.05
			end
			...
			if self.shieldgirl then --Mira's MDR-C
						...
						self.shieldgirl.reload_speed_multiplier = 1.00
			end

			if self.enfieldl22 then --Mira's L22
						self.enfieldl22.has_description = false		
						self.enfieldl22.categories = {
							"assault_rifle"
						}
						self.enfieldl22.CLIP_AMMO_MAX = 30
						self.enfieldl22.AMMO_MAX = 150
						self.enfieldl22.fire_mode_data.fire_rate = 0.08571428571
						self.enfieldl22.auto.fire_rate = 0.08571428571
						self.enfieldl22.panic_suppression_chance = 0.05
						self.enfieldl22.kick = self.stat_info.kick_tables.even_recoil
						self.enfieldl22.supported = true
						self.enfieldl22.ads_speed = 0.220
						...
						self.enfieldl22.reload_speed_multiplier = 1.0
			end

		if self.fckmyfingers then
			self.fckmyfingers.use_data.selection_index = 5
			self.fckmyfingers.supported = true
		end	

			if self.qbz97b then
						self.qbz97b.has_description = false		
						self.qbz97b.categories = {
							"assault_rifle"
						}
						self.qbz97b.CLIP_AMMO_MAX = 30
						self.qbz97b.AMMO_MAX = 150
						self.qbz97b.tactical_reload = 1
						self.qbz97b.fire_mode_data.fire_rate = 0.0923076923
						self.qbz97b.auto.fire_rate = 0.0923076923
						self.qbz97b.panic_suppression_chance = 0.05
						self.qbz97b.kick = self.stat_info.kick_tables.even_recoil
						self.qbz97b.supported = true
						self.qbz97b.ads_speed = 0.220
						self.qbz97b.damage_falloff = {
							start_dist = 2000,
							end_dist = 5500,
							min_mult = 0.5
						}
						...
						self.qbz97b.stats_modifiers = nil
						self.qbz97b.reload_speed_multiplier = 1.01
			end
		...
		if self.romeo870 then
			self.romeo870.recategorize = { "heavy_shot" }
			self.romeo870.damage_type = "shotgun_heavy"
			self.romeo870.damage_type_single_ray = "sniper"
			self.romeo870.tactical_reload = 1
			self.romeo870.fire_mode_data.fire_rate = 0.5
			self.romeo870.fire_rate_multiplier = 0.9
			self.romeo870.CLIP_AMMO_MAX = 6
			self.romeo870.AMMO_MAX = 40
			self.romeo870.supported = true
			self.romeo870.ads_speed = 0.300
			self.romeo870.damage_falloff = {
				start_dist = 700,
				end_dist = 2800,
				min_mult = 0.1333
			}
			...
			self.romeo870.kick = self.stat_info.kick_tables.moderate_kick
			self.romeo870.muzzleflash = "effects/payday2/particles/weapons/big_51b_auto_fps" --"effects/particles/shotgun/shotgun_gen"
			self.romeo870.panic_suppression_chance = 0.05
			self.romeo870.stats_modifiers = nil
			self.romeo870.rays = 9
			self.romeo870.reload_speed_multiplier = 0.8
			self.romeo870.timers.shotgun_reload_enter = 0.3
			self.romeo870.timers.shotgun_reload_first_shell_offset = 0.33
			self.romeo870.timers.shotgun_reload_shell = 0.5666666666666667
			self.romeo870.timers.shotgun_reload_exit_not_empty = 0.3
			self.romeo870.timers.shotgun_reload_exit_empty = 0.7
		end	

		if self.romeo870_sec then
			self.romeo870_sec.recategorize = { "heavy_shot" }
			self.romeo870_sec.damage_type = "shotgun_heavy"
			self.romeo870_sec.damage_type_single_ray = "sniper"
			self.romeo870_sec.tactical_reload = 1
			...
			self.romeo870_sec.kick = self.stat_info.kick_tables.moderate_kick
			self.romeo870_sec.muzzleflash = "effects/payday2/particles/weapons/big_51b_auto_fps" --"effects/particles/shotgun/shotgun_gen"
			self.romeo870_sec.panic_suppression_chance = 0.05
			self.romeo870_sec.stats_modifiers = nil
			self.romeo870_sec.rays = 9
			self.romeo870_sec.reload_speed_multiplier = 1.0
			self.romeo870_sec.timers.shotgun_reload_enter = 0.3
			self.romeo870_sec.timers.shotgun_reload_first_shell_offset = 0.33
			self.romeo870_sec.timers.shotgun_reload_shell = 0.5666666666666667
			self.romeo870_sec.timers.shotgun_reload_exit_not_empty = 0.3
			self.romeo870_sec.timers.shotgun_reload_exit_empty = 0.7
		end	
			...
			if self.alpha57_prim then --RJC9000 and PlayBONK's MW2022 AR57
				...
				self.alpha57_prim.AMMO_MAX = 250
			...
			if self.alpha57 then --RJC9000 and PlayBONK's MW2022 AR57 (in secondary form)
				self.alpha57.categories = {
					"smg",
					"pdw"
				}	
				self.alpha57.recategorize = { "light_smg" }		
				self.alpha57.damage_type = "machine_gun"
				self.alpha57.has_description = true
				self.alpha57.muzzleflash = "effects/payday2/particles/weapons/tkb_muzzle"
				self.alpha57.kick = self.stat_info.kick_tables.even_recoil
				self.alpha57.nato = true
				self.alpha57.tactical_reload = 1
				self.alpha57.FIRE_MODE = "auto"
				self.alpha57.CAN_TOGGLE_FIREMODE = true
				self.alpha57.BURST_FIRE = false
				self.alpha57.fire_mode_data.fire_rate = 0.0857142
				self.alpha57.CLIP_AMMO_MAX = 50
				self.alpha57.AMMO_MAX = 150
				self.alpha57.supported = true
				self.alpha57.ads_speed = 0.260
				self.alpha57.hs_mult = 1.33333
				self.alpha57.armor_piercing_chance = 0.75
				self.alpha57.can_shoot_through_enemy = false
				self.alpha57.can_shoot_through_shield = false
				self.alpha57.can_shoot_through_wall = false
				self.alpha57.damage_falloff = {
					start_dist = 2200,
					end_dist = 5600,
					min_mult = 0.555555
				}
				...
				self.alpha57.stats_modifiers = nil
				self.alpha57.panic_suppression_chance = 0.05
				self.alpha57.timers.reload_exit_empty = 0.6
				self.alpha57.timers.reload_exit_not_empty = 0.9
			end
		...
		--[[     Sora's MODS     ]]--
				...
				if self.m27gear then
					self.m27gear.nato = true
					self.m27gear.recategorize = { "light_ar" }
					self.m27gear.damage_type = "assault_rifle"
					self.m27gear.has_description = false						
					self.m27gear.tactical_reload = 1		
					self.m27gear.AMMO_MAX = 210
					self.m27gear.sounds.fire = "m16_fire_single"
					self.m27gear.sounds.fire_single = "m16_fire_single"
					self.m27gear.sounds.fire_auto = "m16_fire"
					self.m27gear.fire_mode_data.fire_rate = 0.08571428571
					self.m27gear.kick = self.stat_info.kick_tables.moderate_kick
					self.m27gear.supported = true
					self.m27gear.ads_speed = 0.280
					self.m27gear.damage_falloff = {
						start_dist = 2200,
						end_dist = 6500,
						min_mult = 0.4166
					}
					...
					self.m27gear.stats_modifiers = nil
					self.m27gear.reload_speed_multiplier = 1.0
		...
		if self.k3 then
					...
					self.k3.sms = sms_preset.lmg_40
					self.k3.weapon_movement_penalty = sms_preset.lmg_40
					...
					self.k3.reload_speed_multiplier = 1.25
		end

			if self.mg5 then -- Alcat101's HK 121
				self.mg5.categories = {
					"lmg",
					"smg",
					"mmg"
				}
				self.mg5.recategorize = { "heavy_mg" }
				self.mg5.damage_type = "machine_gun"
				self.mg5.nato = true
				self.mg5.lock_slide = true
				self.mg5.sounds.magazine_empty = "wp_rifle_slide_lock"
				self.mg5.sms = sms_preset.lmg_90
				self.mg5.weapon_movement_penalty = sms_preset.lmg_90
				self.mg5.fire_mode_data.fire_rate = 0.1
				self.mg5.CAN_TOGGLE_FIREMODE = false
				self.mg5.CLIP_AMMO_MAX = 50
				self.mg5.AMMO_MAX = 300
				self.mg5.kick = self.stat_info.kick_tables.horizontal_recoil_mg
				self.mg5.always_use_standing = true
				self.mg5.muzzleflash = "effects/payday2/particles/weapons/big_51b_auto_fps"
				self.mg5.supported = true
				self.mg5.ads_speed = 0.440
				self.mg5.damage_falloff = {
					start_dist = 2500,
					end_dist = 7000,
					min_mult = 0.4166
				}
				...
				self.mg5.stats_modifiers = nil
				self.mg5.sounds.spin_start = "wp_m60_reload_lever_release"
				self.mg5.spin_up_shoot = true
				self.mg5.spin_up_t = 0.1
				self.mg5.spin_down_t = 0.00000001
				self.mg5.reload_speed_multiplier = 1.0
				self.mg5.panic_suppression_chance = 0.05
			end
		...
		if self.peepee then
						...
						self.peepee.desc_id = "bm_sr2_sc_desc"
						self.peepee.fire_mode_data.fire_rate = 0.1
						self.peepee.CLIP_AMMO_MAX = 20
						self.peepee.AMMO_MAX = 150
						self.peepee.tactical_reload = 1
						self.peepee.kick = {}
						self.peepee.kick = self.stat_info.kick_tables.even_recoil
						self.peepee.supported = true
						self.peepee.ads_speed = 0.220
						self.peepee.damage_falloff = {
							start_dist = 600,
							end_dist = 3500,
							min_mult = 0.3
						}
						...
						self.peepee.armor_piercing_chance = 0.75
						self.peepee.can_shoot_through_enemy = false
						self.peepee.can_shoot_through_shield = false
						self.peepee.can_shoot_through_wall = false
						self.peepee.hs_mult = 1.2
						self.peepee.stats_modifiers = nil
						self.peepee.panic_suppression_chance = 0.05
						self.peepee.reload_speed_multiplier = 1.0
		end
		...
		if self.fg42 then --Killerwolf's FG42
			...
			self.fg42.sounds.stop_fire = "hk23e_stop"
				...
				if self.heffy_12g then --Saiga-12
					self.heffy_12g.recategorize = { "light_shot" }
					self.heffy_12g.damage_type = "shotgun"
					self.heffy_12g.use_data.selection_index = 1
					self.heffy_12g.damage_type_single_ray = "sniper"
					self.heffy_12g.tactical_reload = 1
					self.heffy_12g.fire_mode_data.fire_rate = 0.2
					self.heffy_12g.CLIP_AMMO_MAX = 5
					self.heffy_12g.AMMO_MAX = 30
					self.heffy_12g.supported = true
					self.heffy_12g.ads_speed = 0.280
					self.heffy_12g.damage_falloff = {
						start_dist = 500,
						end_dist = 3000,
						min_mult = 0.15
					}
					self.heffy_12g.stats = {
						damage = 120,
						spread = 19,
						recoil = 41,
						zoom = 1,
					...
					self.heffy_12g.kick = self.stat_info.kick_tables.vertical_kick
					self.heffy_12g.muzzleflash = "effects/payday2/particles/weapons/big_51b_auto_fps" --"effects/particles/shotgun/shotgun_gen"
					self.heffy_12g.panic_suppression_chance = 0.05
					self.heffy_12g.stats_modifiers = nil
					self.heffy_12g.rays = 9
					self.heffy_12g.reload_speed_multiplier = 1
				end
			...
			if self.kurz_919 then
				...
				if self.mossberg590 then --Silent Enforcer's Mossberg 590
					...
					self.mossberg590.fire_mode_data.fire_rate = 0.5
					self.mossberg590.fire_rate_multiplier = 0.9166666
		...
		if self.toz194 then --Silent Enforcer's TOZ-194
			...
			self.toz194.use_data.selection_index = 1
			...
			self.toz194.AMMO_MAX = 20
			self.toz194.supported = true
			self.toz194.ads_speed = 0.300
			...
			self.toz194.rays = 9
			self.toz194.reload_speed_multiplier = 1.0
		end	

		if self.shot_tf2 then
			self.shot_tf2.recategorize = { "heavy_shot" }
			self.shot_tf2.damage_type = "shotgun_heavy"
			self.shot_tf2.damage_type_single_ray = "sniper"
			self.shot_tf2.tactical_reload = 1
			self.shot_tf2.fire_mode_data.fire_rate = 0.5
			self.shot_tf2.fire_rate_multiplier = 0.9
			self.shot_tf2.CLIP_AMMO_MAX = 6
			self.shot_tf2.AMMO_MAX = 20
			self.shot_tf2.supported = true
			self.shot_tf2.ads_speed = 0.300
			self.shot_tf2.damage_falloff = {
				start_dist = 700,
				end_dist = 2800,
				min_mult = 0.1333
			}
			...
			self.shot_tf2.kick = self.stat_info.kick_tables.moderate_kick
			self.shot_tf2.muzzleflash = "effects/payday2/particles/weapons/big_51b_auto_fps" --"effects/particles/shotgun/shotgun_gen"
			self.shot_tf2.panic_suppression_chance = 0.05
			self.shot_tf2.stats_modifiers = nil
			self.shot_tf2.rays = 9
			self.shot_tf2.reload_speed_multiplier = 0.8
			self.shot_tf2.timers.shotgun_reload_enter = 0.3
			self.shot_tf2.timers.shotgun_reload_first_shell_offset = 0.33
			self.shot_tf2.timers.shotgun_reload_shell = 0.5666666666666667
			self.shot_tf2.timers.shotgun_reload_exit_not_empty = 0.3
			self.shot_tf2.timers.shotgun_reload_exit_empty = 0.7
		end

		if self.shot_tf2_2 then
			self.shot_tf2_2.use_data.selection_index = 5
			self.shot_tf2_2.supported = true
		end	

		if self.qbs then
			self.qbs.recategorize = { "light_shot" }
			self.qbs.damage_type = "shotgun_light"
			self.qbs.damage_type_single_ray = "sniper"
			self.qbs.tactical_reload = 1
			self.qbs.fire_mode_data.fire_rate = 0.2
			self.qbs.fire_rate_multiplier = 0.9
			self.qbs.CLIP_AMMO_MAX = 5
			self.qbs.AMMO_MAX = 30
			self.qbs.supported = true
			self.qbs.ads_speed = 0.250
			self.qbs.damage_falloff = {
				start_dist = 700,
				end_dist = 2800,
				min_mult = 0.1333
			}
			...
			self.qbs.kick = self.stat_info.kick_tables.moderate_kick
			self.qbs.muzzleflash = "effects/payday2/particles/weapons/big_51b_auto_fps" --"effects/particles/shotgun/shotgun_gen"
			self.qbs.panic_suppression_chance = 0.05
			self.qbs.stats_modifiers = nil
			self.qbs.rays = 9
			self.qbs.reload_speed_multiplier = 1.0
			self.qbs.reload_speed_multiplier = 1
			self.qbs.reload_fix_offset = 0.25
			self.qbs.timers.shotgun_reload_enter = 0.3
			self.qbs.timers.shotgun_reload_first_shell_offset = 0.33
			self.qbs.timers.shotgun_reload_shell = 0.5666666666666667
			self.qbs.timers.shotgun_reload_exit_not_empty = 0.2
			self.qbs.timers.shotgun_reload_exit_empty = 1.5
		end	
